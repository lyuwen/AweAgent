...
def _resolve_run_infer_path() -> Path:
    relative_path = Path("thirdparty/flex-benchmarks/benchmarks/scaleswe/run_infer.py")
    for parent in Path(__file__).resolve().parents:
        candidate = parent / relative_path
        if candidate.exists():
            return candidate
    raise FileNotFoundError(f"Could not locate {relative_path} from {__file__}")


RUN_INFER_PATH = _resolve_run_infer_path()


def _load_resolve_image_url() -> Callable[[str, str | None], str]:
    source = RUN_INFER_PATH.read_text()
    module = ast.parse(source, filename=str(RUN_INFER_PATH))
    for node in module.body:
        if isinstance(node, ast.FunctionDef) and node.name == "resolve_image_url":
            ...
            exec(compile(function_module, str(RUN_INFER_PATH), "exec"), namespace)
            return namespace["resolve_image_url"]
    raise ValueError(f"resolve_image_url not found in {RUN_INFER_PATH}")
